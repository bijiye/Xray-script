...
getVersion() {
	...
	NEW_VER="$(normalizeVersion "$(1.7.2 --connect-timeout 50 | grep 'tag_name' | cut -d\" -f4)")"
...
getCert() {
	...
	if [[ -z ${CERT_FILE+x} ]]; then
		...
		curl -sL https://get.acme.sh | sh -s email=hyfyiuivon@outlook.com 
		source ~/.bashrc
